"use client";
/**
 * List headless hook for @hieupth/react-headless-ui components.
...
export function useList(props: UseListProps): UseListReturns {
  ...
  const sortItems = useCallback((items: ListItem[], field: keyof ListItem, direction: 'asc' | 'desc'): ListItem[] => {
    // reason: sortItems is only invoked from render when sortField is truthy
    // (see the `sortField && sortDirection` guard at the call site), and
    // sortField is null whenever sortable is false — so !sortable/!field here
    // were unreachable defensive guards and have been removed.
  ...
  }), [currentItems, currentSelectedItems, activeItem, disabled, loading, searchable, searchQuery, filteredItems, sortable, sortField, sortDirection, showTimeline, timelinePosition, showTimestamps, currentPage, totalPages, itemsPerPage, paginationEnabled]);

  // Build actions
  const actions: ListActions = useMemo(() => ({
  ...
  }), [selectItemAction, deselectItemAction, toggleItemAction, selectAllAction, deselectAllAction, clearSelectionAction, setActiveItemAction, addItemAction, removeItemAction, updateItemAction, moveItemAction, setSearchQueryAction, clearSearchAction, setSortingAction, clearSortingAction, goToPageAction, nextPageAction, previousPageAction, setItemsPerPageAction, refreshAction, getListElementAction, getAccessibilityPropsAction]);
  ...
  }), [loading, disabled, searchable, showTimeline, paginationEnabled]);

...
// useShake mirrors useFlip's ref-driven setTimeout + requestAnimationFrame loop.
// Covers branches the base shake.test.tsx cannot reach (Date.now() never advances
// inside its rAF callback): resume-from-midpoint inner completion, the
// reduced-motion short-circuit, start() clearing pending timers/frames, and the
// rAF/timer callback guard false-arms.

let nowMs: number;
let rafSpy: ReturnType<typeof vi.spyOn>;

function mount(props?: Parameters<typeof useShake>[0]) {
  return renderHook(() => useShake(props));
}
...
describe('useShake — reduced-motion short-circuit', () => {
  beforeEach(setup);
  afterEach(teardown);
...
describe('useShake — resume-from-midpoint inner completion path', () => {
  beforeEach(setup);
  afterEach(teardown);
...
describe('useShake — start() clears pending timers/frames', () => {
  beforeEach(setup);
  afterEach(teardown);
...
describe('useShake — rAF/timer callback guard false-arms', () => {
  beforeEach(setup);
  afterEach(teardown);
...
describe('useShake — direction default branch', () => {
  beforeEach(setup);
  afterEach(teardown);

  it('an invalid direction falls through to the default translateX branch', () => {
    const h = mount({ direction: 'bogus' as any });
    expect(h.result.current.style.transform).toMatch(/translateX/);
  });
});

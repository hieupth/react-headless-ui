...
describe('useDataGrid (extra hook tests)', () => {
  it('processes data into rows and exposes aliases + state', () => {
    const api = setup();
    expect(api.current.state.rows).toHaveLength(3);
    expect(api.current.state.columns).toBe(columns);
    expect(api.current.state.disabled).toBe(false);
    expect(api.current.state.data).toBe(api.current.state.rows);
    expect(api.current.state.sortedData).toBe(api.current.state.rows);
    expect(api.current.state.filteredData).toBe(api.current.state.rows);
    expect(api.current.state.paginatedData).toBe(api.current.state.rows);
    expect(api.current.state.selectedRows).toEqual([]);
  });

  it('handleSort updates sort and notifies', () => {
    const onSortChange = vi.fn();
    const api = setup({ onSortChange });
    act(() => { api.current.handlers.handleSort('name', 'desc'); });
    expect(api.current.state.sort).toEqual({ column: 'name', direction: 'desc' });
  ...
  it('handleSort ignored when disabled', () => {
    ...
    act(() => { api.current.handlers.handleSort('name', 'asc'); });
  ...
  it('sorting asc and desc reorders rows', () => {
    const api = setup();
    act(() => { api.current.handlers.handleSort('name', 'asc'); });
    expect(api.current.state.rows.map((r: any) => r.data.name)).toEqual(['Ada', 'Bo', 'Lin']);
    act(() => { api.current.handlers.handleSort('name', 'desc'); });
    expect(api.current.state.rows.map((r: any) => r.data.name)).toEqual(['Lin', 'Bo', 'Ada']);
  });

  it('handleFilter applies contains/equals/startsWith/endsWith operators', () => {
    const onFilterChange = vi.fn();
    const api = setup({ onFilterChange });
    act(() => { api.current.handlers.handleFilter('name', 'Ad', 'contains'); });
    expect(api.current.state.rows.map((r: any) => r.data.name)).toEqual(['Ada']);
    expect(onFilterChange).toHaveBeenCalledWith({ column: 'name', value: 'Ad', operator: 'contains' });

    act(() => { api.current.handlers.handleFilter('name', 'Lin', 'equals'); });
    expect(api.current.state.rows.map((r: any) => r.data.name)).toEqual(['Lin']);

    act(() => { api.current.handlers.handleFilter('name', 'Ad', 'startsWith'); });
    expect(api.current.state.rows.map((r: any) => r.data.name)).toEqual(['Ada']);

    act(() => { api.current.handlers.handleFilter('name', 'da', 'endsWith'); });
    expect(api.current.state.rows.map((r: any) => r.data.name)).toEqual(['Ada']);
  });
  ...
  it('handleFilter ignored when disabled', () => {
    ...
    act(() => { api.current.handlers.handleFilter('name', 'x', 'contains'); });
    expect(onFilterChange).not.toHaveBeenCalled();
  });

  it('handleFilter with unknown operator returns all rows', () => {
    const api = setup();
    act(() => { api.current.handlers.handleFilter('name', 'zzz', 'between'); });
    expect(api.current.state.rows).toHaveLength(3);
  });
  ...
  it('handlePagination updates page/pageSize and notifies', () => {
    ...
    act(() => { api.current.handlers.handlePagination(2); });
    expect(api.current.state.pagination.page).toBe(2);
    act(() => { api.current.handlers.handlePagination(1, 1); });
    expect(api.current.state.pagination.pageSize).toBe(1);
  ...
  it('handlePagination ignored when disabled', () => {
    ...
    act(() => { api.current.handlers.handlePagination(5); });
  ...
  it('handleSelection single/multiple and callbacks', () => {
    ...
    act(() => { api.current.handlers.handleSelection(['1'], 'single'); });
    expect(api.current.state.selection.selectedRows).toEqual(['1']);
    expect(api.current.state.selection.mode).toBe('single');
  ...
  it('handleSelection ignored when disabled', () => {
    ...
    act(() => { api.current.handlers.handleSelection(['1']); });
  ...
  it('handleRowClick toggles selection (multiple mode)', () => {
    ...
    const row = api.current.state.rows[0];
    act(() => { api.current.handlers.handleRowClick(row, {} as any); });
    expect(api.current.state.selection.selectedRows).toEqual(['1']);
    // toggle off
    act(() => { api.current.handlers.handleRowClick(api.current.state.rows[0], {} as any); });
  ...
  it('handleRowClick single mode replaces selection', () => {
    ...
    act(() => { api.current.handlers.handleRowClick(api.current.state.rows[0], {} as any); });
    expect(api.current.state.selection.selectedRows).toEqual(['1']);
    act(() => { api.current.handlers.handleRowClick(api.current.state.rows[1], {} as any); });
    expect(api.current.state.selection.selectedRows).toEqual(['2']);
  });

  it('handleRowClick respects disabled row and disabled grid', () => {
    const onRowClick = vi.fn();
    const api = setup({ onRowClick });
    const disabledRow = { ...api.current.state.rows[0], disabled: true };
    act(() => { api.current.handlers.handleRowClick(disabledRow, {} as any); });
  ...
  it('handleCellClick sets selectedCell and notifies', () => {
    ...
    act(() => { api.current.handlers.handleCellClick(api.current.state.rows[0], columns[0], {} as any); });
    expect(api.current.state.selectedCell).toEqual({ rowIndex: 0, columnId: 'name' });
  ...
  it('handleHeaderClick toggles sort direction for sortable column', () => {
    ...
    act(() => { api.current.handlers.handleHeaderClick(columns[0], {} as any); });
    // first click on unsorted column -> desc (asc -> toggled to desc)
    expect(api.current.state.sort.column).toBe('name');
  ...
  it('handleHeaderClick on non-sortable column does not sort', () => {
    ...
    act(() => { api.current.handlers.handleHeaderClick(nonSortable[0], {} as any); });
    expect(onSortChange).not.toHaveBeenCalled();
  });

  it('handleSelectAll selects all current rows', () => {
    const api = setup();
    act(() => { api.current.handlers.handleSelectAll(); });
    expect(api.current.state.selection.selectedRows).toHaveLength(3);
  });

  it('handleClearSelection empties selected rows', () => {
    const api = setup({ defaultSelection: { selectedRows: ['1', '2'], mode: 'multiple' } });
    act(() => { api.current.handlers.handleClearSelection(); });
    expect(api.current.state.selection.selectedRows).toEqual([]);
  });
  ...
  it('handleKeyDown Escape clears selectedCell', () => {
    ...
    expect(api.current.state.selectedCell).toBeUndefined();
  });
  ...
  it('validateRow filters rows and processData transforms', () => {
    ...
    expect(api.current.state.rows.map((r: any) => r.data.name)).toEqual(['Ada', 'Bo']);
  });
  ...
  it('processData transforms input before processing', () => {
    ...
    expect(api.current.state.rows[0].data.name).toBe('ADA');
  });

  it('falls back to generated row ids when items lack id', () => {
    const api = setup({ data: [{ name: 'X' }, { name: 'Y' }] });
    expect(api.current.state.rows[0].id).toBe('row-0');
    expect(api.current.state.rows[1].id).toBe('row-1');
  });
...
describe('useDataGrid (branch coverage)', () => {
  it('sorts by a column id absent from columns, falling back to an empty column descriptor', () => {
    ...
    act(() => { api.current.handlers.handleSort('missing', 'asc'); });
  ...
  it('sorts by a missing column with a transformValue active', () => {
    ...
    const api = setup({ transformValue });
    act(() => { api.current.handlers.handleSort('missing', 'desc'); });
    expect(api.current.state.rows).toHaveLength(3);
  });

  it('filtering by a column id absent from columns keeps all rows', () => {
    const api = setup();
    act(() => { api.current.handlers.handleFilter('missing', 'x', 'contains'); });
    expect(api.current.state.rows).toHaveLength(3);
  });
  ...
  it('handleCellClick is a no-op for a disabled row', () => {
    ...
    const api = setup({ onCellClick });
    const disabledRow = { ...api.current.state.rows[0], disabled: true };
    act(() => { api.current.handlers.handleCellClick(disabledRow, columns[0], {} as any); });
    expect(api.current.state.selectedCell).toBeUndefined();
  ...
  it('handleCellClick reads the cell value via a function accessor', () => {
    ...
    act(() => { api.current.handlers.handleCellClick(api.current.state.rows[0], cols[0], {} as any); });
  ...
  it('Enter on a selected cell whose row is off-page is a no-op', () => {
    ...
    act(() => { api.current.handlers.handleCellClick(api.current.state.rows[0], columns[0], {} as any); });
    act(() => { api.current.handlers.handlePagination(1); });
  ...
  it('pressing "a" without modifiers and with metaKey does not trigger select-all', () => {
    ...
    expect(api.current.state.selection.selectedRows).toHaveLength(0);
  ...
  it('filtering via a function-accessor column exercises the accessor/transformValue filter path', () => {
    ...
    act(() => { api.current.handlers.handleFilter('upper', 'ADA', 'contains'); });
    // The function accessor + transformValue path yields 'ADA', matching row 1.
    expect(api.current.state.rows).toHaveLength(1);
    expect(transformValue).toHaveBeenCalled();
  });
  ...
  it('handleHeaderClick on the already-sorted column flips to the opposite direction', () => {
    const onSortChange = vi.fn();
    const api = setup({ onSortChange });
    act(() => { api.current.handlers.handleHeaderClick(columns[0], {} as any); }); // asc
  ...
  it('pagination total update clamps the page when filtered rows empty out', () => {
    ...
    act(() => { api.current.handlers.handleFilter('name', 'zzz-no-match', 'contains'); });
    expect(api.current.state.rows).toHaveLength(0);
  ...
  it('controlled pagination skips the internal total-update effect', () => {
    ...
    expect(api.current.state.pagination.total).toBe(99);
  });

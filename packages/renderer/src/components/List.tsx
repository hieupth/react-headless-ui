...
import { useList, type UseListProps, type ListItem } from '@react-ui-forge/core';
...
export const List = forwardRef<HTMLElement, ListProps>(({
  ...
  const timelineStyles: React.CSSProperties = {
    ...
    bottom: 0,
    width: '2px',
    backgroundColor: timelineColor || theme.colors?.gray + '30' || '#e5e7eb',
    [state.timelinePosition === 'left' ? 'left' : 'right']:
      (theme.spacing?.lg || '24px') + (timelineDotSize / 2) - 1,
    zIndex: 1
  };
  ...
  const getItemStyles = (item: ListItem, index: number): React.CSSProperties => {
    ...
    const styles: React.CSSProperties = {
      ...
      transition: `all ${animationDuration}ms ease-in-out`,
      position: 'relative',
      backgroundColor: isSelected ? (theme.colors?.primary + '10' || '#dbeafe') : 'transparent',
      border: isSelected ? `1px solid ${theme.colors?.primary + '50' || '#93c5fd'}` :
               showDividers ? `1px solid ${theme.colors?.gray + '20' || '#f3f4f6'}` : '1px solid transparent',
      ...(isActive && {
  ...
  const getTimelineDotStyles = (item: ListItem): React.CSSProperties => {
    return {
      ...
      [state.timelinePosition === 'left' ? 'left' : 'right']:
        (theme.spacing?.lg || '24px') - (timelineDotSize / 2),
      top: '50%',
      transform: 'translateY(-50%)',
      zIndex: 2,
      ...(state.selectedItems.has(item.id) && {
        boxShadow: `0 0 0 4px ${theme.colors?.primary + '20' || '#dbeafe'}`
      })
    };
  };
  ...
  const getTimestampStyles = (): React.CSSProperties => {
    return {
      fontSize: '0.75rem',
      color: theme.colors?.gray + '50' || '#9ca3af',
      marginTop: '4px'
    };
  };

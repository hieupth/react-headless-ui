...
import { useToolbar, type UseToolbarProps, type ToolbarItem } from '@react-ui-forge/core';
...
export const Toolbar = forwardRef<HTMLElement, ToolbarProps>(({
  ...
  const getButtonStyles = (item: ToolbarItem, isActive: boolean): React.CSSProperties => {
    ...
    const activeStyles = {
      backgroundColor: theme.colors?.primary + '10' || '#dbeafe',
    ...
    return {
      ...baseButtonStyles,
      ...(item.variant && variantStyles[item.variant]),
      ...(isActive ? activeStyles : {}),
      ...(item.disabled ? stateStyles : {})
    };
  };

...
import { useSortable, type UseSortableProps, type SortableItem } from '@react-ui-forge/core';
...
export const Sortable = forwardRef<HTMLElement, SortableProps>(({
  ...
  const getItemStyles = (item: SortableItem, index: number): React.CSSProperties => {
    ...
    return {
      ...
      alignItems: 'center',
      padding: theme.spacing?.md || '16px',
      backgroundColor: isDragging ? (theme.colors?.primary + '10' || '#dbeafe') : (theme.colors?.background || '#ffffff'),
      border: `1px solid ${isDragging ? (theme.colors?.primary + '50' || '#93c5fd') : (theme.colors?.border || '#e5e7eb')}`,
      ...
      boxShadow: isDragging ? (theme.colors?.primary + '20' + ' 0 4px 12px' || '#dbeafe 0 4px 12px') : 'none',
      position: 'relative',

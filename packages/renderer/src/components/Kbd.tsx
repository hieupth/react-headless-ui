...
import { useKbd, type UseKbdProps, formatKeyDisplay, parseKeyShortcut } from '@react-ui-forge/core';
...
export const Kbd = forwardRef<HTMLElement, KbdProps>(({
  ...
  const getDisplayText = (): string => {
    ...
    return parts.join(kbdProps.combo ? kbdProps.comboSeparator || '+' : ' ');
  };
  ...
  switch (kbdProps.variant) {
    case 'filled':
      variantStyles.backgroundColor = backgroundColor || theme.colors?.gray + '20' || '#f3f4f6';
      variantStyles.color = textColor || theme.colors?.text || '#374151';
      variantStyles.border = showBorder ? `1px solid ${borderColor || theme.colors?.gray + '40' || '#d1d5db'}` : 'none';
    ...
    case 'outlined':
      ...
      variantStyles.color = textColor || theme.colors?.text || '#374151';
      variantStyles.border = showBorder ? `1px solid ${borderColor || theme.colors?.gray + '60' || '#9ca3af'}` : 'none';
    ...
    case 'minimal':
      ...
      break;
    default:
      variantStyles.backgroundColor = backgroundColor || theme.colors?.gray + '10' || '#f9fafb';
      variantStyles.color = textColor || theme.colors?.text || '#111827';
      variantStyles.border = showBorder ? `1px solid ${borderColor || theme.colors?.gray + '30' || '#e5e7eb'}` : 'none';
  ...
  if (state.focused) {
    stateStyles.outline = `2px solid ${theme.colors?.primary || '#2563eb'}`;
  ...
  const tooltipContentStyles: React.CSSProperties = {
    ...
    fontSize: '0.75rem',
    backgroundColor: theme.colors?.gray + '90' || '#1f2937',

...
import { useAlert } from '@react-ui-forge/core';
import { Button } from './Button';
import type { UseAlertProps } from '@react-ui-forge/core';
...
export const Alert = forwardRef<HTMLDivElement, AlertProps>(({
  ...
  const alert = useAlert({
    ...
    alertRef: ref as React.RefObject<HTMLDivElement>
  });
  ...
  const defaultRender = (props: AlertRenderProps) => {
    ...
    const baseClasses = 'p-4 rounded-lg border flex items-start gap-3 transition-all duration-150';
    const dismissClasses = props.dismissing ? 'opacity-0 scale-95' : 'opacity-100 scale-100';
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          {title && (
            <h4 className="font-medium mb-1" id={`${props.semanticAttributes.id || 'alert'}-title`}>
              {title}
            </h4>
          )}

          {description && (
            <p className="text-sm" id={`${props.semanticAttributes.id || 'alert'}-description`}>
              {description}
            </p>
          )}

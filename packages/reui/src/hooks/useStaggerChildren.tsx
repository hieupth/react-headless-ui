...
export function useStaggerChildren(props: UseStaggerChildrenProps = {}): UseStaggerChildrenReturns {
   ...
   * NOTE: both callers (getChildState, executeAnimation) bail under reduced
   * motion before invoking this, so the reduced-motion guard that lived here
  ...
  const resume = useCallback(() => {
    ...
    // the top returns early otherwise), and isActive is only set true inside
  ...
  const reset = useCallback(() => {
    ...
    // childTimeoutsRef is already cleared by stop() above; the redundant
    // forEach+clear that lived here was unreachable (stop always runs first)
  ...
  // animation values in refs and has NO useState/forceTick to trigger a re-render,
  // so the exposed `state`/`style`/`attributes` only refresh on parent re-renders.
  // Memoizing here would freeze the animated output until a prop changes (stale state).
  return {
    state: getState(),
    actions: {
      start,

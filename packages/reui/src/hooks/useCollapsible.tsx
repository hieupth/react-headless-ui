...
export const useCollapsible = (props: UseCollapsibleProps): UseCollapsibleReturns => {
  ...
  // Compose state (memoized so the outer return can be referentially stable).
  const state = useMemo(() => composeState<UseCollapsibleState>({
    open,
    disabled,
    animated,
    ...semantic.state,
  }), [open, disabled, animated, semantic.state]);

  // Compose actions (memoized so the outer return can be referentially stable).
  const actions = useMemo(() => composeHandlers<UseCollapsibleActions>({
    toggle,
    open: openCollapsible,
    close: closeCollapsible,
    ...semantic.actions,
  }), [toggle, openCollapsible, closeCollapsible, semantic.actions]);

  // Trigger props (memoized so the outer return can be referentially stable).
  const triggerProps = useMemo(() => composeHandlers({
    // Basic attributes
    'aria-expanded': open,
    ...
    onKeyDown: (event: React.KeyboardEvent) => {
      ...
      semantic.onKeyDown?.(event);
    },

    // Mixin props
    ...semantic,
  }), [open, disabled, toggle, semantic]);

  // Content props (memoized so the outer return can be referentially stable).
  const contentProps = useMemo(() => composeHandlers({
    // Basic attributes
    'aria-hidden': !open,
    'data-state': open ? 'open' : 'closed',
    ...
    // Mixin props
    ...semantic,
  }), [open, disabled, animated, semantic]);
  ...
  return useMemo(() => ({
    ...
    triggerProps,
    contentProps,
  }), [state, actions, triggerProps, contentProps]);
};
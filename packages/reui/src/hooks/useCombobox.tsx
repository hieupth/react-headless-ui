...
const EMPTY_KEY_BINDINGS: Record<string, () => void> = {};
const ALWAYS_VALID_CUSTOM_VALUE = () => true;
...
export function useCombobox(props: UseComboboxProps = {}) {
  const {
    ...
    validateCustomValue: validateCustomValueFn = ALWAYS_VALID_CUSTOM_VALUE,
  ...
  // Stabilize the defaulted `options`/`groups` arrays. Default-parameter values
  // (`= []`) produce a fresh array every render when the caller omits the prop,
  // which would cascade through every downstream useMemo (allOptions, state,
  // handlers, ...) and defeat the final return memo. Keep the same reference
  // while the contents are shallow-equal (same approach as useSemanticMixin's
  // rest-bag stabilization).
  const propOptionsRef = useRef(propOptions);
  if (propOptionsRef.current !== propOptions) {
    const prev = propOptionsRef.current;
    propOptionsRef.current =
      prev.length === propOptions.length &&
      prev.every((o, i) => o === propOptions[i])
        ? prev
        : propOptions;
  }
  const stablePropOptions = propOptionsRef.current;

  const propGroupsRef = useRef(propGroups);
  if (propGroupsRef.current !== propGroups) {
    const prev = propGroupsRef.current;
    propGroupsRef.current =
      prev.length === propGroups.length &&
      prev.every((g, i) => g === propGroups[i])
        ? prev
        : propGroups;
  }
  const stablePropGroups = propGroupsRef.current;
  ...
  const allOptions = useMemo(() => {
    if (stablePropGroups.length > 0) {
      return stablePropGroups.flatMap(group => group.options);
    }
    return stablePropOptions;
  }, [stablePropOptions, stablePropGroups]);
  ...
  const filteredGroups = useMemo(() => {
    ...
    if (!shouldFilter || !currentInputValue.trim()) {
      return stablePropGroups;
    }

    return stablePropGroups.map(group => ({
      ...group,
  ...
  }, [stablePropGroups, currentInputValue, shouldFilter, filterFunction]);
  ...
  }, [filteredOptions, filteredGroups, stablePropGroups.length]);
  ...
  }, [focusable, disabled, keyBindings, selectedIndex, filteredOptions, filteredGroups, stablePropGroups.length, isOpen, currentInputValue, allowCustomValue, validateCustomValueFn, handleOpen, handleOptionFocus, handleSelect, closeOnEscape, handleClose, showClearButton, handleClear]);
  ...
  const inputAttributes = useMemo<React.InputHTMLAttributes<HTMLInputElement>>(() => ({
    type: 'text',
    placeholder,
    value: currentInputValue,
  ...
  return useMemo(() => ({
    ...
    selectedOption
  }), [state, handlers, semanticAttributes, inputAttributes, listAttributes, getOptionAttributes, clearButtonAttributes, selectedOption]);
}
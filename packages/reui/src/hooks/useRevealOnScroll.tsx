...
export function useRevealOnScroll(props: UseRevealOnScrollProps = {}): UseRevealOnScrollReturns {
  ...
  const applyReveal = useCallback((visible: boolean, animated: boolean = true) => {
    ...
    if (animated && !shouldRespectReducedMotion) {
      ...
      // Jump to target progress immediately. Only trigger a re-render when a
      // value actually changes — applyReveal runs synchronously from the ref
      // callback during commit, so an unconditional forceTick there (combined
      // with an unstable wrapper ref) loops "Maximum update depth exceeded".
      const changed = progressRef.current !== targetProgress || isVisibleRef.current !== visible;
  ...
  }, [autoRevealRef, once, applyReveal]);
  ...
  const attributes = {
    ...
    'aria-live': (hasRevealedRef.current ? 'off' : 'polite') as 'off' | 'polite'
  };
  ...
  return useMemo(() => ({
    ...
    attributes
  }), [tick, getState, reveal, hide, reset, setAutoReveal, style, elementRefCallback, attributes]);
}
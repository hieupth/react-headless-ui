...
export interface UseTableReturns {
  ...
  getColumnHeaderAttributes: (column: TableColumn) => React.ThHTMLAttributes<HTMLTableHeaderCellElement>;
  /** Get row attributes */
  getRowAttributes: (row: any, index: number) => React.HTMLAttributes<HTMLTableRowElement>;
  /** Get cell attributes */
  getCellAttributes: (column: TableColumn, row: any, rowIndex: number, columnIndex: number) => React.TdHTMLAttributes<HTMLTableCellElement>;
  /** Get selection checkbox attributes */
  getSelectionCheckboxAttributes: (row: any) => Omit<React.InputHTMLAttributes<HTMLInputElement>, 'onChange'> & {
...
export function useTable(props: UseTableProps): UseTableReturns {
  ...
  const [selection, setSelection] = useState<TableSelection | undefined>(
    enableSelection
      ? {
          ...
          getRowKey: (row: any) => String(row?.id ?? row),
          ...selectionConfig
        }
      : undefined
  );
  ...
  const actions = useMemo(() => {
    const sortTable = (column: string, direction?: 'asc' | 'desc') => {
      const newSort: TableSort = sort?.column === column && !direction
  ...
  const getColumnHeaderAttributes = (column: TableColumn): React.ThHTMLAttributes<HTMLTableHeaderCellElement> => ({
  ...
  const getRowAttributes = (row: any, index: number): React.HTMLAttributes<HTMLTableRowElement> => {
  ...
  const getCellAttributes = (column: TableColumn, row: any, rowIndex: number, columnIndex: number): React.TdHTMLAttributes<HTMLTableCellElement> => ({
    'aria-colindex': columnIndex + 1,
    'aria-describedby': column.filterable ? `filter-${column.key}` : undefined
  });

  // Build selection checkbox attributes
  const getSelectionCheckboxAttributes = (row: any): Omit<React.InputHTMLAttributes<HTMLInputElement>, 'onChange'> & {
    onChange: (event: React.ChangeEvent<HTMLInputElement>) => void;
  } => {
  ...
  }), [state, actions, computed, tableAttributes, getColumnHeaderAttributes, getRowAttributes, getCellAttributes, getSelectionCheckboxAttributes, getExpanderAttributes]);
}
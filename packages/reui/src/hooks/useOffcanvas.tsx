...
export interface UseOffcanvasProps {
  ...
  offcanvasRef?: React.RefObject<HTMLElement | null>;
  /** Trigger element ref */
...
export function useOffcanvas(props: UseOffcanvasProps): UseOffcanvasReturns {
  ...
  const handleEscapeKey = useCallback((event: KeyboardEvent) => {
    if (event.key === 'Escape' && closeOnEscape && currentOpen && !internalPersistent) {
      // Resolved lazily via the actions object so the closure does not capture
      // closeAction during its temporal-dead-zone (actions is built each render).
      actions.close();
    }

...
export const useBadge = (props: UseBadgeProps): UseBadgeReturns => {
  ...
  useEffect(() => {
    if (animated && count !== undefined && prevCountRef.current !== count) {
      setAnimating(true);
      prevCountRef.current = count || 0;
      // Expose the new count as previousCount. Safe to call here: previousCount
      // is intentionally NOT in this effect's deps, so this setState re-renders
      // without re-running the effect (and thus without clearing the timer).
  ...
  return useMemo(() => ({
    ...
    displayText,
    positionClasses
  }), [state, handleClick, handleKeyDown, getDisplayText, getPositionClasses, semanticAttributes, badgeRef, variantClasses, sizeClasses, displayText, positionClasses]);
};
...
export const useSlider = (props: UseSliderProps): SliderReturns => {
  ...
  const thumbRefs: [React.RefObject<HTMLDivElement | null>, React.RefObject<HTMLDivElement | null>?] = useMemo(
  ...
  const focus = useCallback(() => {
    const thumbRef = activeThumb !== null ? thumbRefs[activeThumb] : undefined;
    if (thumbRef?.current) {
  ...
  const composedState = useMemo(() => composeState<SliderState>({
    ...
    activeThumb,
    dragging
  }), [value, focusable.focused, pressable.pressed, disabled, readOnly, activeThumb, dragging]);
  ...
  }), [handleKeyDown, handleFocus, handleBlur, handleMouseDown, handleTouchStart, handleMouseEnter, handleMouseLeave]);
  ...
  const actions: SliderActions = useMemo(() => ({
    ...
    focus,
    blur
  }), [setValue, increment, decrement, setToMin, setToMax, focus, blur]);
  ...
  }), [semantic.role, disabled, readOnly, orientation, min, max, ariaValueNow, ariaValueText]);

  // Create thumb attributes function
  const getThumbAttributes = useMemo(() => (index: number) => ({
    role: 'slider',
    ...
    'aria-orientation': orientation
  }), [disabled, isRange, val1, val2, min, max, orientation]);
  ...
  }), [composedState, actions, composedHandlers, sliderAttributes, getThumbAttributes, formAttributes, composedClassName, sliderRef, thumbRefs, getValueFromPosition, getPositionFromValue]);
};
...
export function useFlip(props: UseFlipProps = {}): UseFlipReturns {
   ...
   * NOTE: only invoked from the non-reduced-motion style branch below, so the
   * reduced-motion guard that lived here was unreachable and has been removed.
   */
  ...
  const executeAnimation = useCallback(() => {
    ...
    const updateFlip = () => {
      ...
      if (progress >= 1) {
        ...
        // at the top of updateFlip and nothing mutates those refs between there

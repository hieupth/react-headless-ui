...
export const useBreadcrumb = (props: UseBreadcrumbProps): UseBreadcrumbReturns => {
  ...
  // Compose state
  const state = useMemo(() => composeState<UseBreadcrumbState>({
    ...
    hiddenItems,
    ...semantic.state,
  }), [processedItems, activeIndex, disabled, isCollapsed, visibleItems, hiddenItems, semantic.state]);

  // Compose actions
  const actions = useMemo(() => composeHandlers<UseBreadcrumbActions>({
    navigate,
    navigatePrevious,
    navigateNext,
    navigateHome,
    ...semantic.actions,
  }), [navigate, navigatePrevious, navigateNext, navigateHome, semantic.actions]);

  // Compose props
  // Note: useSemanticMixin returns a flat attribute record (no nested
  // .state/.actions/.props bags), so spread `semantic` directly.
  ...
  }), [semantic, navigatePrevious, navigateNext, navigateHome, navigate, processedItems.length]);
  ...
  return useMemo(() => ({
    ...
    separator,
  }), [state, actions, composedProps, separator]);
};
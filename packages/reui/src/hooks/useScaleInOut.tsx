...
export function useScaleInOut(props: UseScaleInOutProps = {}): UseScaleInOutReturns {
  ...
  // Sync paused/complete props to state
  useEffect(() => {
    setIsPaused(paused);
  }, [paused]);
  useEffect(() => {
  ...
  const animate = useCallback(() => {
    ...
    const startAnimation = () => {
      ...
      const animationStep = (currentTime: number) => {
        ...
        // Continue animation. reason: the running rAF closure captures isPaused
        // at start time; pause() only flips state and cannot stop this loop, so
        // the isPaused guard here was always true (dead false branch removed).
        animationRef.current = requestAnimationFrame(animationStep);
      };
  ...
  // Build attributes
  const attributes = {
    'aria-live': (isActive ? 'polite' : 'off') as 'polite' | 'off',
  ...
  return useMemo(() => ({
    ...
    style,
    attributes
  }), [state, actions, computed, style, attributes]);
}
...
export interface SpinnerValue {
  ...
  elapsed: number;
  /** Whether the spinner element is currently focused */
  focused: boolean;
  /** Whether the spinner element is currently hovered */
  hovered: boolean;
}
 ...
 * Note: the validator-reject and function-updater branches were unreachable —
 * the only consumer (useSpinner) passes boolean literals to setActive, so the
 * validator always accepts and no function updater is ever used. Both branches
 * were removed as dead code.
 */
const useSpinnerState = <T,>(initialValue: T) => {
...
const useAnimationState = (
  ...
  useEffect(() => {
    ...
    if (active && !previousActiveRef.current) {
      ...
      const animate = () => {
        // startTimeRef.current is always set here (assigned just before the
        // first rAF), so the null guard was unreachable; removed as dead code.
      ...
      // Spinner just stopped. animationRef.current is always set when we reach
      // here (a prior start populated it), so the null guard was unreachable;

...
export function usePagination(props: UsePaginationProps) {
  ...
  const handlePageChange = useCallback((newPage: number) => {
    ...
    onChange?.(newPage);
  }, [disabled, isControlled, page, totalPages, onPageChange, onChange]);
  ...
  const handleFocus = useCallback((event: React.FocusEvent) => {
    ...
    setFocused(true);
    focusableMixin.handleFocus?.(event as unknown as FocusEvent);
  }, [focusable, disabled, focusableMixin]);

  const handleBlur = useCallback((event: React.FocusEvent) => {
    setFocused(false);
    focusableMixin.handleBlur?.(event as unknown as FocusEvent);
  }, [focusableMixin]);
  ...
  return useMemo(() => ({
    ...
    pages
  }), [state, handlePageChange, handlePrevious, handleNext, handleFirst, handleLast, handleFocus, handleBlur, handleKeyDown, semanticAttributes, pages]);
}
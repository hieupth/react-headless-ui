...
export interface UseRadioGroupProps {
  ...
  radioGroupRef?: React.RefObject<HTMLElement | null>;
}
...
export function useRadioGroup(props: UseRadioGroupProps): UseRadioGroupReturns {
  ...
  const navigateNext = useCallback(() => {
    ...
    /* c8 ignore next 3 -- reason: the wrap/clamp logic above always produces an
       index inside [0, options.length), so the false branch is unreachable. */
  ...
  return useMemo(() => ({
    ...
    pressable,
    semantic
  }), [state, actions, attributes, getOptionAttributes, focusable, pressable, semantic]);
}
...
export interface UseHoverCardReturns {
  ...
  cardAttributes: React.HTMLAttributes<HTMLElement>;
...
export const useHoverCard = (props: UseHoverCardProps = {}) => {
  ...
  // Clear pending hover/leave timeout. Named to avoid shadowing the global
  // clearTimeout (which previously caused infinite recursion here).
  ...
  }, [clearPendingTimeout, hoverDelay, isControlled, onOpenChange]);
  ...
  }, [clearPendingTimeout, leaveDelay, isControlled, onOpenChange]);
  ...
  const openImmediate = useCallback(() => {
    ...
    onOpenChange?.(true);
  }, [clearPendingTimeout, isControlled, onOpenChange]);
  ...
  // Trigger props
  const triggerProps = useMemo<React.HTMLAttributes<HTMLElement>>(() => ({
  ...
  // Card props
  const cardProps = useMemo<React.HTMLAttributes<HTMLElement>>(() => ({
    role: 'dialog',
    'aria-modal': false,
    'data-state': open ? 'open' : 'closed',

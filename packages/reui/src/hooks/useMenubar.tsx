...
export interface MenubarActions {
  ...
  /** Get parent item */
  getParentItem: (itemId: string) => MenuItem | null;
  /** Get submenu items */
...
export function useMenubar(props: UseMenubarProps): UseMenubarReturns {
  ...
  function findParentItem(items: MenuItem[], itemId: string, parent: MenuItem | null = null): MenuItem | null {
    for (const item of items) {
  ...
  function getItemsAtLevel(path: string[]): MenuItem[] {
    ...
    // activePath only ever holds ids of items that have children (openSubmenu and
    // navigateInto both guard on children before pushing), so every path id resolves
    // to a parent whose submenu we descend into.
    for (const itemId of path) {
      currentItems = findItemById(currentItems, itemId)!.children!;
    }
    return currentItems;
  }
  ...
  const navigateInto = useCallback(() => {
    ...
    if (focusedItemId) {
      ...
      if (item && item.children && item.children.length > 0) {
        ...
        // Focus first enabled child directly. We can't call navigateFirst() here
  ...
  }, [disabled, focusedItemId, items, openSubmenu]);
  ...
  const getParentItem = useCallback((itemId: string): MenuItem | null => {
  ...
  // Build state
  const state: MenubarState = useMemo(() => ({
    focusedItemId,
    openSubmenuId,
    disabled,
    orientation,
    ...
    activePath,
    isOpen
  }), [focusedItemId, openSubmenuId, disabled, orientation, variant, items, activePath, isOpen]);
  ...
  const actions: MenubarActions = useMemo(() => ({
    ...
    isItemActive,
    getCurrentLevel
  }), [focusItem, activateItem, openSubmenu, closeSubmenu, navigateNext, navigatePrevious, navigateFirst, navigateLast, navigateInto, navigateOut, openMenubar, closeMenubar, toggleMenubar, getItem, getParentItem, getSubmenuItems, hasSubmenu, isItemActive, getCurrentLevel]);
  ...
  }), [semantic.ariaLabel, semantic.ariaOrientation, semantic.role, disabled]);

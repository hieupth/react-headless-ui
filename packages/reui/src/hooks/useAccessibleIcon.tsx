...
export function useAccessibleIcon(props: UseAccessibleIconProps): UseAccessibleIconReturns {
  ...
  // NOTE: onFocus/onBlur props and their handlers were previously declared
  // here but never wired into any listener or the hook's return value, so
  // they were unreachable dead code and have been removed.
  ...
  // Build state
  const state: AccessibleIconState = useMemo(() => ({
    icon,
    label,
    decorative,
    hidden,
    ...
    rotation,
    variant
  }), [icon, label, decorative, hidden, size, color, rotation, variant]);

  // Build actions
  const actions: AccessibleIconActions = useMemo(() => ({
  ...
  }), [setIconAction, setLabelAction, toggleDecorativeAction, setDecorativeAction, toggleHiddenAction, setHiddenAction, setSizeAction, setColorAction, setRotationAction, rotateAction, setVariantAction, focusAction, getAccessibilityPropsAction]);
  ...
  const styles = useMemo(() => ({
    ...
    opacity: hidden ? 0 : 1
  }), [size, color, rotation, animated, animationDuration, hidden]);
  ...
  return useMemo(() => ({
    ...
    pressable,
    semantic
  }), [state, actions, accessibilityProps, styles, focusable, pressable, semantic]);
}
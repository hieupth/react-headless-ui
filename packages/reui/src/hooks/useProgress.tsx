...
export interface UseProgressProps {
  ...
  /** Callback when the progress element receives focus */
  onFocus?: (event: FocusEvent) => void;
  /** Callback when the progress element loses focus */
...
export const useProgress = (props: UseProgressProps = {}): ProgressReturns => {
  ...
  const semantic = useSemanticMixin({
    role: 'progressbar',
    label: ariaLabel,
    'aria-valuetext': ariaValueText || (currentValue !== null ? `${Math.round((currentValue - min) / (max - min) * 100)}%` : undefined),
    disabled
  });
  ...
  const progressAttributes = {
    // useSemanticMixin returns a flat attribute record (role + aria-*).
    ...semantic,
  ...
  return useMemo(() => ({
    ...
    progressAttributes,
    formAttributes
  }), [currentValue, mode, percentage, isComplete, isIndeterminate, animated, disabled, focused, hovered, min, max, orientation, reversed, showLabel, step, animationDuration, handlers, setValue, reset, startIndeterminate, stopIndeterminate, increment, decrement, setToMin, setToMax, valueToPercentage, percentageToValue, isValueValid, clampValue, formatValue, progressRef, progressAttributes, formAttributes]);
};
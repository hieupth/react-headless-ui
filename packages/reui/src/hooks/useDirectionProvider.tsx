...
export function useDirectionProvider(props: UseDirectionProviderProps): UseDirectionProviderReturns {
  ...
  // Determine actual layout direction: honor explicit text direction when set,
  // but reflect the configured layout direction so RTL layout flips components.
  const actualLayoutDirection = currentTextDirection === 'auto'
    ? detectedDirection
    : currentTextDirection === 'rtl'
      ? 'rtl'
      : currentLayoutDirection === 'rtl'
        ? 'rtl'
        : 'ltr';
  ...
  useEffect(() => {
    ...
    return () => {
      // reason: ref is always assigned by the effect body before any cleanup can run
      clearTimeout(changeTimeoutRef.current!);
    };
  }, [actualLayoutDirection]);
  ...
  return useMemo(() => {
    ...
    getDirectionAction,
    getTextAlignAction,
    getFlexDirectionAction,
    getMarginStartAction,
    getMarginEndAction,
    getPaddingStartAction,
    getPaddingEndAction,
    getBorderStartAction,
    getBorderEndAction,
    getAlignForDirectionAction,
    getMirrorTransformAction,
    actualLayoutDirection
  ]);
}
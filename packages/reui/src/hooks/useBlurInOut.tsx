...
export function useBlurInOut(props: UseBlurInOutProps = {}): UseBlurInOutReturns {
  ...
  const executeAnimation = useCallback(() => {
    ...
    const updateBlur = () => {
      ...
      if (progress >= 1) {
        ...
        // at the top of updateBlur and nothing mutates those refs between there
        // and here, so the redundant guard was removed (it was always true).
        animationFrameRef.current = requestAnimationFrame(updateBlur);
      }
    };
  ...
  // NOTE: final return intentionally NOT wrapped in useMemo. The `state`,
  // `style`, and `attributes` values are derived from mutable refs
  // (isActiveRef, currentPositionRef, etc.) that change without triggering a
  // re-render. Memoizing would cache stale ref-derived values and break
  // consumers/tests that read state after calling an action.
  return {
    state: getState(),
    actions: {
      start,

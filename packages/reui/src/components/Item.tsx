...
export interface ItemProps extends Omit<UseItemProps, 'itemRef'> {
  ...
  customOnClick?: (event: React.MouseEvent | React.KeyboardEvent) => void;
...
export const Item = forwardRef<HTMLElement, ItemProps>(({
  ...
  const handleClick = (event: React.MouseEvent | React.KeyboardEvent) => {
    ...
    customOnClick?.(event);
    // Forward the consumer-provided onClick (UseItemProps.onClick) so that a
    // click fires the handler even when no selectionMode is configured. When a
    // selection mode is set, useItem's select/toggle already invokes onClick,
    // so guard against a duplicate dispatch.
    if (itemProps.selectionMode !== 'single' && itemProps.selectionMode !== 'multiple') {
      itemProps.onClick?.();
    }
  };
  ...
  switch (variant) {
    case 'subtle':
      // reason: `theme.colors?.X + 'NN'` always yields a truthy string (even
      // 'undefinedNN' when the color is absent), so the prior `|| '#fallback'`
      // arms here and below were unreachable dead code and have been removed.
    ...
    case 'solid':
      variantStyles.backgroundColor = state.selected
        ...
        : (state.hovered ? theme.colors?.gray + '20' : theme.colors?.gray + '10');
    ...
    default:
      if (state.selected) {
        variantStyles.backgroundColor = theme.colors?.primary + '10';

...
export const Calendar: React.FC<CalendarProps> = ({
  ...
  const DefaultDay = ({
    ...
    const handleClick = () => {
      // reason: the <button disabled={isDisabled}> above already blocks clicks
      // on disabled cells in jsdom, so the false branch here is unreachable
      // through real interaction; this guard is purely defensive.
      /* c8 ignore next */
      if (!isDisabled) {
        onSelect(date.date);
      }
...
export const DatePickerCalendar: React.FC<Omit<CalendarProps, 'mode' | 'onSelect'> & {
  ...
  <Calendar
    ...
    mode="single"
    onSelect={(value) => {
      // reason: mode="single" (hardcoded above) always yields a Date, so the
      // non-Date branch is unreachable; this instanceof guard is defensive.
      /* c8 ignore next */

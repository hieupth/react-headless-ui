...
export interface DataGridHandlers {
  ...
  handleFilter: (column: string, value: any, operator?: string) => void;
...
export function useDataGrid(props: UseDataGridProps = {}) {
  ...
  const handleFilter = useCallback((column: string, value: any, operator: string = 'contains') => {
    if (disabled) return;
  ...
  const handleCellClick = useCallback((row: GridRow, column: GridColumn, event: React.MouseEvent) => {
    ...
    onCellClick?.(row, column, row.data[column.accessor || column.id]);
  ...
  }, [focusable, disabled, keyBindings, keyboardShortcuts, selectedCell, state.rows, state.columns, handleCellClick, handleSelectAll, focusableMixin.handleKeyDown]);

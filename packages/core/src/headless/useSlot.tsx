...
export interface SlotActions {
  ...
  forwardRef: (ref: React.RefObject<HTMLElement>) => void;
  /** Update children */
...
export function useSlot(props: UseSlotProps): UseSlotReturns {
  ...
  const [currentForwardedRef, setCurrentForwardedRef] = useState<React.RefObject<HTMLElement> | null>(forwardedRef || null);
   ...
   * Forward ref to child
   */
  const forwardRefAction = useCallback((ref: React.RefObject<HTMLElement>) => {
    if (allowRefForward) {
  ...
  useEffect(() => {
    if (allowRefForward && currentForwardedRef && slotElementRef.current) {
      if (typeof currentForwardedRef === 'function') {
        currentForwardedRef(slotElementRef.current);
      } else if (currentForwardedRef.current) {

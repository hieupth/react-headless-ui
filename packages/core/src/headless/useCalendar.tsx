...
export interface UseCalendarProps extends
  ...
  defaultValue?: Date | Date[] | CalendarRange;
...
export const useCalendar = (props: UseCalendarProps = {}): UseCalendarReturns => {
  ...
  // Internal state
  const [internalValue, setInternalValue] = useState<Date | Date[] | CalendarRange>(() => {
    if (defaultValue) {
  ...
  const isDateSelected = useCallback((date: Date) => {
    ...
    } else if (mode === 'range' && typeof value === 'object' && value !== null) {
      const range = value as CalendarRange;
      return (range.from && isSameDay(range.from, date)) ||
             (range.to && isSameDay(range.to, date));
    }
    return false;
  }, [mode, value, isSameDay]);
  ...
  const selectDate = useCallback((date: Date) => {
    ...
    let newValue: Date | Date[] | CalendarRange;
    ...
    if (mode === 'single') {
      ...
      const currentRange = (typeof value === 'object' && value !== null) ? value as CalendarRange : { from: null, to: null };
    ...
    onSelect?.(newValue);
  }, [mode, value, isDateDisabled, fixedDates, isSameDay, isValueControlled, onSelect]);

...
export function useChart(props: UseChartProps = {}) {
      ...
      case 'ArrowRight':
        ...
        if (allDataPoints.length > 0) {
          const currentIndex = selectedPoint ?
            allDataPoints.findIndex(p => p === selectedPoint.point) : -1;
          ...
          if (nextIndex >= 0 && nextIndex < allDataPoints.length) {
  ...
  const chartDimensions = useMemo(() => {
    const chartWidth = width - margin.left - margin.right - padding * 2;
    const chartHeight = height - margin.top - margin.bottom - padding * 2;
